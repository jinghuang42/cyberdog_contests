...
#                continue
            ...
            #vyaw_cmd = y_err * 0.05 
                       # --- 【核心：两侧深度居中逻辑】 ---
            # 1. 限制 TOF 的感知上限（防止扫到远处的干扰）
            # 假设桥宽的一半是 0.3m, 我们把读数限制在 0.5m 以内
            safe_l = min(tl, 0.5) 
            safe_r = min(tr, 0.5)
            
            # 2. 计算两侧差值 (左 - 右)
            # 如果 左边 > 右边：说明右边窄，狗偏右了。
            # diff 为正，vy_cmd 为正，驱动狗向左平移
            diff_y = safe_l - safe_r

            # 3. 设置 2 厘米死区：偏差极小时，强制保持直线不晃动
            if abs(diff_y) < 0.02:
                vy_cmd = 0.0
            else:
                # 使用较小的 P 增益 (0.25)，让平移变得极其柔和
                vy_cmd = diff_y * 0.25 
            ...
            if dist_moved < 3.5:
                ...
                diff_lidar = L - R
                vy_cmd = diff_lidar * 0.15  # 雷达量程大，增益调小防止晃动
            ...
            if abs(y_err) > 1.5:
                vy_cmd *= 0.1  # 大幅削弱侧移，消除“斜着走”的视觉感
                vx_cmd = 0.07  #减速方便掉头
            else:
                vx_cmd = 0.12

            #安全限制
            vy_cmd = max(min(vy_cmd, 0.03), -0.03)  #限制最大平移，防止侧翻
    ...
    # ==================== 主运行逻辑 ====================
    def wall_hug_advance(self, side, target_dist, wall_gap=0.35, stop_at_wall=False):
        """
        side: 'left' 或 'right'
        target_dist: 设定的前进距离
        wall_gap: 离墙距离
        stop_at_wall: 是否根据前方墙面停止（用于最后一段）
        """
        # --- 阶段 1：原地侧向位移 (只横移，不前进) ---
        print(f'>>> [幕布-位移] 正在向{side}侧原地平移...')
        while True:
            scan = self.lidar.get_scan()
            curr_yaw = self.odom.y
            ...
            current_gap = scan[0] if side == 'left' else scan[2] 
            gap_error = current_gap - wall_gap
            
            # 对齐精度：偏差小于 5 厘米就停止原地横移
            if abs(gap_error) < 0.05:
                break
            
            y_err = (self.init_angle - curr_yaw + 180) % 360 - 180
            # 计算横移速度 (vx 固定为 0)
            vy_cmd = (-gap_error * 0.30) if side == 'right' else (gap_error * 0.30)
            vy_cmd = max(min(vy_cmd, 0.06), -0.06)
            
            self.publish(7, v_x=0.0, v_y=vy_cmd, v_yaw=y_err * 0.05)
            time.sleep(0.02)

        self.publish(0); time.sleep(0.2) 

        # --- 阶段 2：锁定距离并前进 ---
        print(f'>>> [幕布-前进] 正在以贴{side}侧模式起步...')
        start_xyz = list(self.odom.xyz)
        while True:
            scan = self.lidar.get_scan()
            curr_xyz = self.odom.xyz
            ...
            # --- 【核心修改：判定退出条件】 ---
            if stop_at_wall:
                # 如果是最后一段，检测前方 0.45 米停止
                if F < 0.45:
                    print(f'>>> [探测] 到达赛道尽头，前方距离 {F:.2f}m')
                    break
            else:
                # 否则，按设定的距离（1.2m 或 1.0m）停止
            ...
            current_gap = scan[0] if side == 'left' else scan[2]
            gap_error = current_gap - wall_gap
            vy_cmd = (-gap_error * 0.4) if side == 'right' else (gap_error * 0.4)
            ...
            self.publish(7, v_x=vx_cmd, v_y=vy_cmd, v_yaw=y_err * 0.05)
            
            if self.life_count % 20 == 0:
                print(f">>> [前进] 模式:{'测距' if not stop_at_wall else '测墙'} | 位移:{dist_moved:.2f}m | 距墙:{current_gap:.2f}")
            time.sleep(0.02)
    ...
    def run_competition(self):
        while True:
            ...
            # ==================== Stage 6: 全自动关卡链 (唯一入口) ====================
                ...
                # 第一个幕布 (贴右)
                self.wall_hug_advance(side='right', target_dist=1.2, wall_gap=GAP_VAL, stop_at_wall=False)
                # 第二个幕布 (贴左)
                self.wall_hug_advance(side='left', target_dist=1.0, wall_gap=GAP_VAL, stop_at_wall=False)
                # 第三个幕布 (贴右)
                # 这里的 target_dist 随便填一个大数即可（比如 99），因为它会根据墙面停止
                self.wall_hug_advance(side='right', target_dist=99.0, wall_gap=GAP_VAL, stop_at_wall=True)
                # ==================== 最终左转逻辑 ====================
                print('>>> [终点转向] 执行最后的左转...')
                target_yaw = (self.odom.y + 90) % 360
                while True:
                ...
                print('>>> [系统] 🎉 所有关卡圆满通过！')
                self.stage = 1
                self.task_id = 15 # 切换到新ID，防止再次进入 Stage 6
