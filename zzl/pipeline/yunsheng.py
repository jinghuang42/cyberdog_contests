...
#                continue
            ...
            if dist_moved < 3.5:
                ...
                if not stage_2_triggered:
                    # ==================== 【核心新增：切换点原地朝前并居中】 ====================
                    print(">>> [切换] 到达3.5m，正在原地执行身子对正与居中...")
                    for _ in range(60): # 原地校准 1.2 秒
                        # 获取实时数据
                        cur_tl, cur_tr = self.lidar.get_tof()
                        cur_y = self.odom.y
                        if cur_y is None: continue
                        # 计算原地偏差
                        原地_y_err = (at_direction - cur_y + 180) % 360 - 180
                        原地_diff = cur_tr - cur_tl # 继续使用TOF做精准平衡
                        # 执行动作：vx=0 确保原地，vy居中，vyaw朝前
                        self.publish(7, v_x=0.0, v_y=max(min(原地_diff*0.3, 0.04), -0.04),
                                     v_yaw=原地_y_err*0.06, h=0.08, p_z=0.07)
                        time.sleep(0.02)
                    # =========================================================================
                    print(">>> [切换] 姿态已回正，开启雷达高抬腿模式...")
    ...
    def wall_hug_advance(self, side, target_dist, wall_gap=0.30, stop_at_wall=False, switch_dist=None):
        """
        switch_dist: 如果设定了该值(例如1.1)，则在走够该距离后切换为雷达左右居中模式
        """
        # --- 1. 强制起步定身 ---
        for _ in range(35):
        ...
        while True:
            scan = self.lidar.get_scan()
            if scan is None: self.publish(0); time.sleep(0.02); continue
            current_gap = scan[0] if side == 'left' else scan[2]
            if abs(current_gap - wall_gap) < 0.04: break
        ...
        while True:
            ...
            # --- 【核心修改：判定切换与退出】 ---
            if stop_at_wall:
                # 只有在走完 target_dist 且 距离墙面 < 0.30m 才停止
            ...
            if switch_dist and dist_moved >= switch_dist:
                # 模式切换：此时 L 和 R 应该是走廊两侧墙壁，执行居中
                diff_y = L - R
                vy_cmd = diff_y * 0.15 # 雷达居中增益
                mode_label = "雷达居中"
            else:
            ...
            vyaw_cmd = y_err * 0.06
            if abs(y_err) > 1.5:
                vy_cmd, vx_cmd = 0.0, 0.07 # 身子歪了禁止横移
            else:
                vx_cmd = 0.15

            self.publish(7, v_x=vx_cmd, v_y=max(min(vy_cmd, 0.04), -0.04), v_yaw=vyaw_cmd, h=0.08, p_z=0.07)
            if self.life_count % 20 == 0:
